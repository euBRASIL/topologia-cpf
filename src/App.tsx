...
const NetworkTopology = () => {
  ...
  const [newNode, setNewNode] = useState<Node>({ id: '', cpf: '', connections: [] });
  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      <h1 className="text-3xl font-bold mb-4">Network Topology</h1>
      <div className="flex flex-wrap justify-center mb-4">
        {nodes.map((node) => (
          ...
          >
            ...
            <p className="text-sm">Connections: {node.connections.join(', ')}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap justify-center">
        {links.map((link) => (
          <div key={`${link.source}-${link.target}`} className="bg-gray-300 p-2 m-2">
            <p className="text-sm">
